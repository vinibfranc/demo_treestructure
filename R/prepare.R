...
system(glue("{IQTREE2_BIN} -s data/seqs1_up_2020_02_29.fa -m GTR+F+R3 -nt AUTO -ntmax 3 -B 1000 -nm 500 > /dev/null 2>&1 &"))
...
plot(mltr_root, show.tip.label = T)
...
trestruct_res_nobt <- trestruct(timetr2_phylo, minCladeSize=30, nodeSupportValues=FALSE, level=0.01, ncpu=NCPU) #6 pts
...
seqs2_fa_filt1 <- seqs2_fa[!(rownames(seqs2_fa) %in% c(seqs1_to_rm,to_rm$taxon)),] #6748, remove previous seqs with too many gaps and outliers from treedater
...
md2 <- md2[md2$strain %in% rownames(seqs2_fa_filt1),]
...
# running
system(glue("{IQTREE2_BIN} -s data/seqs2_adj1_up_2020_03_15.fa -m GTR+F+R3 -nt AUTO -ntmax 3 -B 1000 -nm 500 > /dev/null 2>&1 &"))
...
rm_long_br <- c("KOR/CNUHV03/2020|MT678839.1|2020-03-11", "POL/IHG_PAS_25_P7726/2020|MZ047087.1|2020-03-15", "JPN/JP_HiroFH173c/2020|OM816720.1|2020-03-02",
																...
																"Japan/Donner382/2020|BS001051.1|2020-02-24")

mltr_add_root <- drop.tip(phy=mltr_add_root, tip=rm_long_br)
